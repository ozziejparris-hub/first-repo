...
**ELO recalculation schedule:** Full 6-dimensional recalculation runs automatically every Sunday at 03:00 UTC via the `polymarket-sunday-elo.timer` systemd timer, which calls `scripts/run_sunday_elo.sh` → `scripts/recalculate_comprehensive_elo.py` directly — **not** through `daily_maintenance.py` (which runs separately, daily at 06:00 UTC including Sundays, and has its own additional Sunday-only steps, but does not itself perform the full ELO recalculation). Last manual full recalculation: April 30 2026.

#!/usr/bin/env python3
"""
SEGMENT 2 -- THE WRITE (2026-08-21-discovery-gap-closure-prereg.md,
832198d as amended, SS C segmented execution model + combo/parlay
carve-out).

Reads the FIXED, pre-materialized segment2 list (segment2_list.json,
60,500 markets) -- segment 1's original remaining tail
(segment1_list.json indices 6000-102999, 97,000 markets) with the
combo/parlay carve-out predicate already applied at materialization time
(15,208 excluded), then truncated to this session's runway-sized 60,500.

THE EXCLUSION IS APPLIED AT THE LIST-MATERIALIZATION STAGE, not as a
runtime filter in this driver. This driver's code is otherwise identical
in structure to segment1_write.py -- it does not modify
tranche2_write.py (6061a5a) in any way; it imports ACCEPTED_REASONS and
atomicity_count from it directly, exactly as segment1_write.py did.

Batch numbering: segment 2's batches (1-121) are a NEW, independent
sequence -- NOT a continuation of segment 1's 1-206 numbering. See the
pre-write fingerprint's "exclusion_and_numbering" note for why.

Batches of 500, atomic checkpoint (write-to-temp-then-os.replace) after
each, to data/checkpoints/segment2_checkpoint.json. Resumable against the
...
SEGMENT_LIST_PATH = Path(__file__).parent / "segment2_list.json"
CHECKPOINT_PATH = REPO_ROOT / "data" / "checkpoints" / "segment2_checkpoint.json"
...
def main():
    ...
    print(f"[SEGMENT2-WRITE] Loaded fixed segment list: {len(segment)} markets "
          f"(combo/parlay cohort already excluded at materialization time: "
          f"{segment_doc['combo_excluded_count']} excluded from a {segment_doc['segment1_remaining_tail_count']}-market tail)")

    conn = _get_connection()

    pre_atomicity = atomicity_count(conn)
    print(f"[SEGMENT2-WRITE] Pre-write check_resolution_write_atomicity: {pre_atomicity}")
    if pre_atomicity != 0:
        print("[SEGMENT2-WRITE] ABORT CONDITION fired BEFORE any write. Not proceeding.")
        conn.close()
        sys.exit(2)

    checkpoint = load_checkpoint()
    if checkpoint is not None:
        print(f"[SEGMENT2-WRITE] RESUMING from checkpoint: batches_completed="
        ...
        print("[SEGMENT2-WRITE] No checkpoint found -- starting fresh (segment 2 batch 1).")
        resolved_ids = set()
    ...
    for batch_num in range(batches_completed, n_batches):
        ...
        if fire - now <= MAINTENANCE_STOP_MARGIN:
            print(f"[SEGMENT2-WRITE] *** MAINTENANCE-WINDOW STOP: {fire.isoformat()} is within "
        ...
        print(f"\n[SEGMENT2-WRITE] === Batch {batch_num + 1}/{n_batches} "
            ...
            if clob_response is None:
                ...
                if classification == "resolved":
                    try:
                        ...
                        print(f"[SEGMENT2-WRITE] *** sqlite3 exception on write for {market_id}: {e} ***")
        ...
        if aborted:
            print(f"[SEGMENT2-WRITE] Aborted mid-batch {batch_num + 1}: {abort_reason}")
            break
        ...
        print(f"[SEGMENT2-WRITE] Batch {batches_completed}/{n_batches} done: "
        ...
        print(f"[SEGMENT2-WRITE] Checkpoint written: {CHECKPOINT_PATH}")
    ...
    print(f"\n[SEGMENT2-WRITE] {status}")
    if aborted:
        print(f"[SEGMENT2-WRITE] Abort reason: {abort_reason}")
    print(f"[SEGMENT2-WRITE] Batches completed: {batches_completed}/{n_batches}")
    print(f"[SEGMENT2-WRITE] Cumulative processed: {cumulative_processed}/{len(segment)}")
    print(f"[SEGMENT2-WRITE] Cumulative tally: {cum_tally}")
    print(f"[SEGMENT2-WRITE] Cumulative accepted={cum_accepted} rejected={cum_rejected}")
    print(f"[SEGMENT2-WRITE] Cumulative reasons: {cum_reasons}")
    print(f"[SEGMENT2-WRITE] Cumulative elapsed: {cumulative_elapsed:.1f}s")

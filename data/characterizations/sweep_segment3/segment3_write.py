#!/usr/bin/env python3
"""
SEGMENT 3 -- THE WRITE (2026-08-21-discovery-gap-closure-prereg.md,
23630ee as amended, SS C segmented execution model + combo/parlay
carve-out).

Reads the FIXED, pre-materialized segment3 list (segment3_list.json,
93,000 markets) -- a live, current-state SS C query (not a segment-1-list
derived tail the way segment 2 was), combo/parlay carve-out applied
directly in SQL, excluding every market_id already walked by tranche 2
(5,000), segment 1's first 6,000 walked entries, and segment 2's full
60,500. See segment3_materialize.py for the exact query and the tranche-1
exclusion caveat (accepted, not silently ignored).

THIS DRIVER IS IDENTICAL IN STRUCTURE TO segment2_write.py, WITH EXACTLY
ONE FUNCTIONAL CHANGE: terminal-signal wiring (Fix 2,
2026-08-23-sweep-safety-fixes.md, 72e7337) -- data/characterizations/
sweep_common/sweep_terminal_signal.py's write_terminal_marker() and
send_telegram_terminal() are now called at the three exit points the
module's own docstring specifies: normal/abort/maintenance-stop
completion (covers COMPLETE, ABORTED, MAINTENANCE-STOPPED, STOPPED
(incomplete) via the same status computation segment2_write.py already
had) and a new top-level try/except Exception around the batch loop for
an unhandled exception (status=EXCEPTION), which segment2_write.py did
not have at all. Every other line of logic -- batching, pacing, abort
conditions, checkpointing, imports from tranche2_write.py -- is otherwise
unchanged from segment2_write.py; only path constants (segment3 instead
of segment2) and the two lines that were their own copy-paste targets
(segment number in print prefixes) differ beyond that.

Batch numbering: segment 3's batches (1-186) are a NEW, independent
sequence -- NOT a continuation of segment 1 or segment 2's numbering.

Batches of 500, atomic checkpoint (write-to-temp-then-os.replace) after
each, to data/checkpoints/segment3_checkpoint.json. Resumable against the
fixed list via a persisted resolved_market_ids skip-list, populated
whenever a reason is in ACCEPTED_REASONS (the defect-2 fix, same as
tranche 2, segment 1 and segment 2).
...
SEGMENT_LIST_PATH = Path(__file__).parent / "segment3_list.json"
CHECKPOINT_PATH = REPO_ROOT / "data" / "checkpoints" / "segment3_checkpoint.json"
TERMINAL_MARKER_PATH = REPO_ROOT / "data" / "checkpoints" / "segment3_terminal_marker.json"
...
def _emit_terminal(status, batches_completed, n_batches, cumulative_processed, cumulative_tally, reason=None):
    ...
    send_telegram_terminal(
        f"[SWEEP] segment3 {status}: {batches_completed}/{n_batches} batches, "
...
def main():
    ...
    print(f"[SEGMENT3-WRITE] Loaded fixed segment list: {len(segment)} markets "
    ...
    print(f"[SEGMENT3-WRITE] Pre-write check_resolution_write_atomicity: {pre_atomicity}")
    if pre_atomicity != 0:
        print("[SEGMENT3-WRITE] ABORT CONDITION fired BEFORE any write. Not proceeding.")
        conn.close()
        sys.exit(2)

    checkpoint = load_checkpoint()
    if checkpoint is not None:
        print(f"[SEGMENT3-WRITE] RESUMING from checkpoint: batches_completed="
        ...
        print("[SEGMENT3-WRITE] No checkpoint found -- starting fresh (segment 3 batch 1).")
        resolved_ids = set()
    ...
    try:
        for batch_num in range(batches_completed, n_batches):
            ...
            if fire - now <= MAINTENANCE_STOP_MARGIN:
                print(f"[SEGMENT3-WRITE] *** MAINTENANCE-WINDOW STOP: {fire.isoformat()} is within "
            ...
            print(f"\n[SEGMENT3-WRITE] === Batch {batch_num + 1}/{n_batches} "
                ...
                if clob_response is None:
                    ...
                    if classification == "resolved":
                        try:
                            ...
                            print(f"[SEGMENT3-WRITE] *** sqlite3 exception on write for {market_id}: {e} ***")
            ...
            if aborted:
                print(f"[SEGMENT3-WRITE] Aborted mid-batch {batch_num + 1}: {abort_reason}")
                break
            ...
            print(f"[SEGMENT3-WRITE] Batch {batches_completed}/{n_batches} done: "
            ...
            print(f"[SEGMENT3-WRITE] Checkpoint written: {CHECKPOINT_PATH}")
    ...
    print(f"\n[SEGMENT3-WRITE] {status}")
    if aborted:
        print(f"[SEGMENT3-WRITE] Abort reason: {abort_reason}")
    print(f"[SEGMENT3-WRITE] Batches completed: {batches_completed}/{n_batches}")
    print(f"[SEGMENT3-WRITE] Cumulative processed: {cumulative_processed}/{len(segment)}")
    print(f"[SEGMENT3-WRITE] Cumulative tally: {cum_tally}")
    print(f"[SEGMENT3-WRITE] Cumulative accepted={cum_accepted} rejected={cum_rejected}")
    print(f"[SEGMENT3-WRITE] Cumulative reasons: {cum_reasons}")
    print(f"[SEGMENT3-WRITE] Cumulative elapsed: {cumulative_elapsed:.1f}s")

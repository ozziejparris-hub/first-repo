...
def main():
    ...
    if choice == "1":
        ...
        csv_path = os.path.join(reports_dir, f"trader_performance_7days_{datetime.now().strftime('%Y%m%d')}.csv")
        ...
        metrics = analyzer.analyze_trader_performance(days_filter)
        analyzer.generate_report(metrics, days_filter)
        csv_path = os.path.join(reports_dir, f"trader_performance_30days_{datetime.now().strftime('%Y%m%d')}.csv")
        analyzer.save_to_csv(metrics, csv_path)
        ...
        for period, days in [("7 days", 7), ("30 days", 30), ("All time", None)]:
            ...
            csv_path = os.path.join(reports_dir, f"trader_performance_{suffix}_{datetime.now().strftime('%Y%m%d')}.csv")
            analyzer.save_to_csv(metrics, csv_path)
            print("\n" + "="*70 + "\n")
